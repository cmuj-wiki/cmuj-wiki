...
4. COLOR-AWARE: Wykrywa zajęcia grupowane kolorem tła (gdzie tylko pierwsza grupa ma tekst)
"""
...
def get_cell_color(cell):
    ...
    try:
        ...
        # Priority: indexed > rgb > theme (indexed is most common in this XLSX)

        # Check indexed color first (most common in this workbook)
        if hasattr(fg_color, 'indexed') and fg_color.indexed is not None:
            try:
                return f"indexed{int(fg_color.indexed)}"
            except:
                pass
        ...
        if hasattr(fg_color, 'rgb') and fg_color.rgb:
            try:
                if isinstance(fg_color.rgb, str):
                    return f"rgb_{fg_color.rgb}"
                return f"rgb_{str(fg_color.rgb)}"
            except:
                pass

        # Then check theme
        if hasattr(fg_color, 'theme') and fg_color.theme is not None:
            try:
                theme_val = int(fg_color.theme)
                tint_val = float(getattr(fg_color, 'tint', 0.0))
                return f"theme{theme_val}_tint{tint_val}"
            except:
                pass

        # Legacy index attribute
        if hasattr(fg_color, 'index') and fg_color.index:
            try:
                return f"index{int(fg_color.index)}"
            except:
                pass
...
def find_class_text_for_color(ws, column, target_row, target_color):
    ...
    SAFETY: Limits search to nearby rows and fails fast if color changes,
    preventing cross-contamination from distant groups.
    """
    if not target_color:
        return None

    # Only search within a reasonable range (max 10 rows up)
    # This prevents picking up classes from completely different group blocks
    MAX_LOOKUP_DISTANCE = 10

    for row_num in range(target_row - 1, max(0, target_row - MAX_LOOKUP_DISTANCE), -1):
        cell = ws.cell(row=row_num, column=column)
        cell_color = get_cell_color(cell)

        # CRITICAL: If color doesn't match, stop immediately
        # This prevents cross-group contamination
        if cell_color != target_color:
            return None

        # Found matching color with text
        if cell.value:
            return cell.value
...
def parse_time_from_column(col_idx, header_row):
    ...
    - Kolumna D (index 4) = 8:00 (header shows "8")
    ...
    # Kolumna D (index 4) to początek dnia o 8:00
    # Header structure: B=h, C=empty, D=8:00, E=8:15, ..., H=9:00
    offset = col_idx - 4
...
def infer_duration_from_cells(ws, cell, row_cells, col_idx):
    ...
    2. Count consecutive cells with same color (color-based spanning)
    3. Look at when the next class starts (scan forward)
    4. Use subject-based defaults as fallback
    ...
    # Strategy 2: Color-based duration (most reliable)
    # Count all consecutive cells with the same color
    cell_color = get_cell_color(cell)
    if cell_color:
        slots_in_block = 1  # Current cell
        MAX_COLOR_SCAN = 20  # Don't scan beyond 5 hours

        for offset in range(1, MAX_COLOR_SCAN):
            next_col = cell.column + offset
            next_cell = ws.cell(row=cell.row, column=next_col)

            # Stop if we hit a cell with text (next class)
            if next_cell.value and str(next_cell.value).strip():
                break

            # Check if color changed (end of current class block)
            next_color = get_cell_color(next_cell)
            if next_color != cell_color:
                # Color changed - this is the end of the block
                break

            slots_in_block += 1

        # Return duration based on colored cells
        if slots_in_block > 1:
            duration = slots_in_block * 15
            return duration

    # Strategy 3: Scan forward to find next non-empty cell
    ...
    # Strategy 4: Use subject-based defaults
    ...
    for sheet_name in wb.sheetnames:
        ...
        for row in ws.iter_rows(min_row=header_row_idx + 1):
            ...
            for col_offset, cell in enumerate(row_cells[2:], start=0):  # Start from column C
                ...
                # Get cell color (for color-based grouping)
                cell_color = get_cell_color(cell)

                # Get cell value - if empty but colored, look up the text
                cell_value = cell.value
                if not cell_value and cell_color:
                    # Color-based grouping: find text from another row with same color
                    cell_value = find_class_text_for_color(ws, cell.column, row_idx, cell_color)

                # Skip if no value found (neither text nor colored)
                if not cell_value:
                    continue

                # Parse the cell content
                parsed = parse_class_cell(cell_value)
                if not parsed:
                    continue

                # Calculate time from column position
                # cell.column is 1-based (A=1, B=2, C=3, etc.)
                # Column C (3) = 8:00, each column = 15 minutes
                hour, minute = parse_time_from_column(cell.column, header_row)
                ...
                # CRITICAL FIX: Mark entire duration span as processed
                # This prevents empty colored cells in the span from being re-processed
                # as separate classes via color lookup
                num_slots = duration_minutes // 15
                for span_offset in range(num_slots):
                    span_col = cell.column + span_offset
                    span_cell = ws.cell(row=cell.row, column=span_col)
                    processed_coords.add(span_cell.coordinate)

                # Dopasuj lokalizację z mapy adresów
                if not parsed['location']:
                    # Hardcoded override: Etyka Sem takes place at Katedra Biochemii
                    if 'etyka' in parsed['subject'].lower():
                        parsed['location'] = 'Katedra Biochemii Lekarskiej (KBL), ul. Kopernika 7'
                    else:
